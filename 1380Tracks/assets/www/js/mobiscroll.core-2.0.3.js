    ...
    function Scroller(elm, settings) {
        ...
        */
        that.show = function () {
            ...
            var html = '<div class=" jqm "> <div class="dw dwbg dwi"><div class="dwwr">' + (s.headerText ? '<div class="dwv"></div>' : '');
                        for (var i = 0; i < s.wheels.length; i++) {
                            ...
                            for (var label in s.wheels[i]) {
                                html += '<td><div class="dwwl dwrc dwwl' + l + '">' + '<div class="dwl">' + label + '</div><div class="dww dwrc" style="height:' + (s.rows * hi) + 'px;min-width:' + s.width + 'px;"><ul>';
                                // Create wheel values
